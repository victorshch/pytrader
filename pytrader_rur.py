...
class Listener(QtCore.QObject):
  ...
  def tradeForward(self):
    ...
    print str(QtCore.QDateTime.currentDateTime().toString()) + ": exploring usd->btc->rur->usd arbitrage opportunity"
    
    usdToSpend = min(self.balance_usd, maxUSD, a1 / k1 * min(X, maxBTC, self.balance_btc), a1 * a2 / (k1 * k2) * min(Y, maxRUR, self.balance_rur), a1 * a2 / (k1 * k2 * k3) * Z)
  ...
  def onTimer(self):
    ...
    self.balance_rur = float(info.balance_rur)
    ...
    profit1 = k1 * k2 * k3 * b2 / (a1 * a3)  - 1.0
    profit2 = k1 * k2 * k3 * b1 * b3 / a2  - 1.0
    #if profit1 > 0:
      #self.tradeForward()
    #elif profit2 > 0:
      #self.tradeBackward()
